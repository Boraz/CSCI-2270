...
void copy_list(const node* source_head_ptr, node*& dest_head_ptr, node*& dest_tail_ptr)
{
 ...
 /*  templates
struct node{ ItemType data;
	node* next;
	node* prev;
	}  node<int>* head_ptr = nullptr // let the node know its data type by using <>
	node<double> node<char> // not always efficient, not for mission critical stuff
	*/
